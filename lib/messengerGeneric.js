...
let messengerGeneric = {
    ...
    sendPropertyResults( recipientId, results, centerLoc ) {

        //show success text
        this.sendTextMessage( recipientId, "Found " + results.length + " development applications near the area you selected:" );

        //get all results
        var locations = results.map( result => result.application.address );

        //show map with all points
        this.sendImageMessage( recipientId, googleMapImg.getManyMapUrl( locations, centerLoc ) );
     ...
     *
     */
    sendMainOptions( recipientId ) {
        console.log( 'send Main Options' );
        ...
        var messageData = {
            ...
            message: {
                text: "What would you like to know about development in your area?",
                metadata: "DEVELOPER_DEFINED_METADATA",
                quick_replies: [ {
                    "content_type": "text",
    ...
    sendTrendingIssuesBrisbane( recipientId, cityName ) {
        ...
        elements.push( {
            title: "Heritage Building Demolition",
            subtitle: "O’Reilly’s Bonded & Free Stores",
            item_url: 'https://brisbanedevelopment.com/oreillys-bonded-stores/',
            image_url: 'https://brisbanedevelopment.com/wp-content/uploads/2013/12/1006288.jpg',
        } );

        this.sendTrendingIssuesMessage( recipientId, elements );
    },
    ...
    sendTrendingIssuesMessage( recipientId, elements ) {
        ...
        this.callSendAPI( messageData );
        setTimeout( () => {
            this.sendMainOptions( recipientId );
        }, 1000 );
    },
    ...
    verifyRequestSignature( req, res, buf ) {
        ...
        if ( !signature ) {
            ...
            if ( signatureHash != expectedHash ) {
                throw new Error( "Couldn't validate the request signature. " + envVars.APP_SECRET + " " + signatureHash + " " + expectedHash );
            }
        }
    },
    /*
