            ...
            messageAttachments.forEach( attachment => {
                if ( attachment.type === 'location' ) {
                    ...
                    request.get( 'https://api.planningalerts.org.au/applications.js', {
                        ...
                        var filtered = body.filter( item => count++ < 50 );

                        console.log( 'Got', body.length, 'results. Filtered to', filtered.length );
                        async.forEach( body, dataFinder.findMoreData, ( error, results ) => {
                            return messengerGeneric.sendPropertyResults( senderID, results );
                        } );
                    } );
                }
            } );
    ...
    receivedPostback( event ) {
        ...
        case 'MORE_INFO':
            ...
            request.get( 'https://api.planningalerts.org.au/applications.js', {
                ...
                body.filter( item => item.application.address === payload.address );
                messengerGeneric.sendSingleApplication( senderID, body[ 0 ].application );
            } );
            break;
        default:
